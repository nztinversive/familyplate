...
- App Review status: Current live version is `1.16.9` build `60` in `READY_FOR_SALE` after the manual release request was accepted on 2026-07-03. Local release source `ea32aa9fd64fe77c47fc7f52776bfd64f41d8477` built and pushed `1.16.10` build `61`, `npx eas-cli@latest metadata:push --profile production --non-interactive` synced the App Store version, `npx eas-cli@latest submit --platform ios --profile production --path /Users/noahthies/.codex/worktrees/49ea/familyplate/apps/mobile/build-1783088429303.ipa --non-interactive` uploaded the same IPA on 2026-07-03, and `fastlane deliver submit_build` then submitted that exact processed build for App Review. Direct App Store Connect read-back at 2026-07-03T16:02:42Z shows version `1.16.10` build `61` in `WAITING_FOR_REVIEW` with manual release.
